...
export function GameBoard({ context, myPlayerId, children }: GameBoardProps) {
    ...
    return (
        <div className="flex flex-col h-screen bg-gray-900 text-white relative">
            <header className="p-4 bg-gray-800 flex justify-between items-center shadow-lg mb-4">
                <h1 className="text-xl font-bold text-yellow-500">17보 마작 (17-Steps Mahjong)</h1>
            ...
            <div className="flex-1 flex flex-col items-center justify-center relative w-full max-w-6xl mx-auto">
                ...
                {/* Opponent Area (Top) */}
                {otherPlayerId && (
                    <div className="w-full flex flex-col items-center mb-8 opacity-90 transition-opacity p-4 border border-gray-700 rounded-lg bg-gray-800/50">
                        <div className="flex items-center gap-4 mb-2">
                            <div className="w-10 h-10 bg-red-800 rounded-full flex items-center justify-center font-bold border-2 border-red-600 shadow-md">
                                {otherPlayerId.slice(0, 1).toUpperCase()}
                            </div>
                            <span className="text-lg font-bold text-red-200">{otherPlayerId} (Opponent)</span>
                            <span className="font-mono text-yellow-400 bg-black/40 px-2 py-0.5 rounded">
                                {context.scores[otherPlayerId] || 0} pts
                            </span>
                            {/* Dealer Indicator */}
                            {context.dealer === otherPlayerId && (
                                <span className="ml-2 px-2 py-0.5 bg-yellow-600 text-black text-xs font-bold rounded">DEALER</span>
                            )}
                        </div>
                ...
                {/* My Area (Bottom) */}
                <div className="w-full flex flex-col items-center mt-auto p-6 bg-gradient-to-t from-gray-800 to-transparent rounded-t-3xl">
                    {/* My Discards */}
                    <div className="mb-6 relative group">
                        <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 bg-gray-800 px-3 py-1 rounded text-xs text-gray-400 border border-gray-700 shadow-sm">
                            MY DISCARDS
                        </div>
                        <div className="p-4 bg-gray-800/80 rounded-xl shadow-inner border border-gray-700/50 backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-6 w-full justify-between max-w-3xl">
                        <div className="flex items-center gap-3">
                            <div className="w-12 h-12 bg-blue-600 rounded-full flex items-center justify-center font-bold text-xl border-4 border-blue-400 shadow-lg relative overflow-hidden">
