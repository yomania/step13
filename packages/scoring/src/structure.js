...
function indexToTile(idx) {
    if (idx >= 27) {
        return { suit: 'z', rank: (idx - 27 + 1), isRed: false };
    }
    const suits = ['man', 'pin', 'sou'];
    const sIdx = Math.floor(idx / 9);
    const rank = (idx % 9) + 1;
    return { suit: suits[sIdx], rank: rank, isRed: false };
}
/**
...
function analyzeStructure(hand) {
    ...
    function solve(currentIndices, currentBlocks) {
        // Find first non-zero index
        let idx = 0;
        while (idx < 34 && currentIndices[idx] === 0)
            idx++;
        if (idx === 34) {
            ...
            for (const b of currentBlocks) {
                if (b.type === 'sequence' || b.type === 'triplet')
                    m++;
                else if (b.type === 'pair')
                    p++;
                else if (['ryanmen', 'kanchan', 'penchan'].includes(b.type))
                    t++;
            }
    ...
    solve([...indices], []);
    // Also calculate Chiitoitsu shanten
    let pairCount = 0;
    let uniqueCount = 0;
    for (const count of indices) {
        if (count >= 1)
            uniqueCount++;
        if (count >= 2)
            pairCount++;
    }
    // Chiitoitsu shanten: 6 - number of unique pairs. 
    // If uniqueCount < 7, we need more unique tiles (not usually an issue in 13 tile hand).
    const chiitoiShanten = 6 - pairCount;
    // Use the best shanten
    const finalShanten = Math.min(bestResult.shanten, chiitoiShanten);
    return {
        shanten: finalShanten,
