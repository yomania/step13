...
import { analyzeStructure, HandStructure } from './structure';
...
const WEIGHTS: Record<Difficulty, HeuristicWeights> = {
    ...
    HARD: {
        ...
        koutsu: 50,
        ryanmen: 80,
        speedvsValue: 0.8 // Value Focused
    }
};

export function evaluateHandQuality(hand: Tile[], difficulty: Difficulty = 'MEDIUM', doraIndicators: Tile[] = []): number {
...
function findPotentialBlocks(tiles: Tile[]): import('./structure').Block[] {
    const blocks: import('./structure').Block[] = [];
